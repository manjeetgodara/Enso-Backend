...
class TransitionApproval(BaseModel):
    ...
    transactioner = models.ForeignKey(app_config.USER_CLASS, verbose_name=_('Transactioner'), null=True, blank=True, on_delete=SET_NULL, related_name='transaction_approvals')
    transaction_date = models.DateTimeField(null=True, blank=True)

    status = models.CharField(_('Status'), choices=STATUSES, max_length=100, default=PENDING)

    permissions = models.ManyToManyField(app_config.PERMISSION_CLASS, verbose_name=_('Permissions'), related_name='transaction_approvals_permissions')
    groups = models.ManyToManyField(app_config.GROUP_CLASS, verbose_name=_('Groups'), related_name='transaction_approvals_groups')
    users = models.ManyToManyField("myauth.users",verbose_name=_('Users'), related_name='transaction_approvals_users')
